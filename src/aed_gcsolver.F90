...
!                                                                              !
   conversion = 0.0  !## CAB [-Wmaybe-uninitialized]
...
!------------------------------------------------------------------------------!
 SUBROUTINE GetListOfGeochemDiagnostics(speciesOutput,derivedNames)
    ...
    !-- to remember some of the FeII & FeIII species since it uses these for
    !-- calculating the FeII oxidation, and FeIII photoreduction etc
    ...
    WRITE(*,"(11X,'aed_gcsolver # diagnostics: ',I3,/)")nGCDerivedVars
    DO i = 1,nGCDerivedVars
      WRITE(*,"(12X,'Var: ',A10)")TRIM(tmpNames(i))
    END DO
...
! returnGCDerivedVector                                                        !
!                                                                              !
! Rotuine that searches for the name and resturns the vector of derived        !
! variables values. Current available deived variables include:                !
!   a) Any simulated aqueous species                                           !
!   b) pCO2                                                                    !
!   c) Saturation Indicies (SO) for mineral phases                             !
!------------------------------------------------------------------------------!
 FUNCTION returnGCDerivedVector(Name, derivedVector) RESULT(status)            !
   !-- Incoming                                                                !
   CHARACTER(LEN=*),               INTENT(IN)  :: Name                         !
   !-- Outgoing                                                                !
   REAL, INTENT(OUT) :: derivedVector
   INTEGER                                     :: status                       !
   !-- Local                                                                   !
   CHARACTER(*), PARAMETER ::      THIS_PROC = "returnGCDerivedVector"         !
   INTEGER :: i                                                                !
   ...
   ELSE
     DO i = 1,nGCDerivedVars
      ...
      IF(derivedGCList(i) > 0) THEN
        IF(TRIM(Name) == TRIM(allSpecies(derivedGCList(i))%Name)) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
          EXIT
        END IF

      ELSE IF(derivedGCList(i) <= -1 .AND. derivedGCList(i) >= -5) THEN
        IF(TRIM(Name) == "SI_PP1" .AND. derivedGCList(i) == -1) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
        ELSE IF(TRIM(Name) == "SI_PP2" .AND. derivedGCList(i) == -2) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
        ELSE IF(TRIM(Name) == "SI_PP3" .AND. derivedGCList(i) == -3) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
        ELSE IF(TRIM(Name) == "SI_PP4" .AND. derivedGCList(i) == -4) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
        ELSE IF(TRIM(Name) == "SI_PP5" .AND. derivedGCList(i) == -5) THEN
          derivedVector = REAL(derivedGCVals(i))
          status = 1
        END IF
...
!------------------------------------------------------------------------------!
FUNCTION CalcIAPforPP(PICHMIndex) RESULT(SI_PPX)                               !
   !-- Incoming                                                                !
   INTEGER  :: PICHMIndex                                                   !
   !-- Outgoing                                                                !
   DOUBLETYPE  :: SI_PPX                                                       !
   ...
   IF(allComponents(PICHMIndex)%CompType == PUREPHASE) THEN
     ...
     SI_PPX = ( allComponents(PICHMIndex)%Value -                              &
                allComponents(PICHMIndex)%Total ) / LOG(REAL(10.0,DP))
